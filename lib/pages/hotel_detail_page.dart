...
//   final String hotelName = "Ocean View Resort";
//   final double rating = 4.5;
//   final String location = "Bali, Indonesia";
//   final String description =
//       "Experience luxury and comfort with breathtaking ocean views. "
//       "Our resort offers world-class amenities, including a private beach, infinity pool, "
...
//     final selectedRoom = selectedRoomIndex != null ? rooms[selectedRoomIndex!] : null;
...
//                   ElevatedButton(
//                     onPressed: () {
//                       // You could navigate to a booking page here
//                     },
...
//                 crossAxisAlignment: CrossAxisAlignment.start,
//                 children: [
//                   Text(hotelName, style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
//                   SizedBox(height: 8),
//                   Row(
//                     children: [
...
//                   // Amenities
//                   Text("Amenities", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
//                   SizedBox(height: 10),
...
//                   // Room Options
//                   Text("Available Rooms", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
//                   SizedBox(height: 16),
//                   Column(
//                     children: rooms.asMap().entries.map((entry) {
//                       int index = entry.key;
//                       var room = entry.value;
//                       bool isSelected = selectedRoomIndex == index;
...
//                                       backgroundColor: isSelected ? Colors.deepPurple : Colors.grey[600],
//                                     ),
...
//   Widget build(BuildContext context) {
//     final selectedRoom =
//         selectedRoomIndex != null ? widget.rooms[selectedRoomIndex!] : null;

//     return Scaffold(
//       appBar: AppBar(
//         title:
//             const Text("Hotel Details", style: TextStyle(color: Colors.white)),
//         backgroundColor: Colors.blue,
//         centerTitle: true,
//         iconTheme: const IconThemeData(color: Colors.white),
//       ),
...
//                   ...widget.amenities.map((item) => Padding(
//                         padding: const EdgeInsets.symmetric(vertical: 4.0),
//                         child: Row(
//                           children: [
...
//             iconTheme: const IconThemeData(color: Colors.white),
//           ),
//           bottomNavigationBar: selectedRoomIndex != null
//               ? Container(
//                   padding: const EdgeInsets.all(16),
//                   color: Colors.white,
//                   child: Row(
...
//                     children: [
//                       // Text(
//                       //   "${rooms[selectedRoomIndex!]['type']} - \$${rooms[selectedRoomIndex!]['price']}/night",
//                       //   style: const TextStyle(fontWeight: FontWeight.bold),
//                       // ),
//                       ElevatedButton(
//                         onPressed: () {
...
//                               color: Colors.red, size: 20),
//                           const SizedBox(width: 4),
//                           // Text(hotel.locationId), // Ganti dengan nama lokasi jika sudah diambil
...
//         .collection('rooms')
//         .where('hotelId', isEqualTo: widget.hotelId) // Ambil kamar berdasarkan hotelId
//         .get();
...
//                     crossAxisAlignment: CrossAxisAlignment.start,
//                     children: [
//                       Text(hotel.name, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
//                       const SizedBox(height: 8),
//                       Row(
//                         children: [
//                           const Icon(Icons.star, color: Colors.orange, size: 20),
...
//                               children: [
//                                 Image.asset(
//                                   "assets/images/hotel.png", // Ganti dengan gambar kamar jika ada
//                                   height: 150,
//                                   width: double.infinity,
//                                   fit: BoxFit.contain,
//                                 ),
...
//                                     crossAxisAlignment: CrossAxisAlignment.start,
//                                     children: [
//                                       Text(room.type, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
//                                       const SizedBox(height: 8),
...
//   DateTime? startDate;
//   DateTime? endDate;
//   int guestCount = 1; // Default guest count
//   double? totalPrice; // Total price calculation
...
//   void _calculateTotalPrice(double price) {
//     if (startDate != null && endDate != null && guestCount > 0) {
...
//       } else {
//         setState(() {
//           totalPrice = null; // Reset total price if dates are invalid
//         });
//       }
//     }
//   }
...
//   //     setState(() {
//   //       startDate = picked;
//   //       _calculateTotalPrice(_rooms[selectedRoomIndex!].price); // Recalculate total price
//   //     });
//   //   }
//   // }
...
//               _rooms[selectedRoomIndex!].price); // Recalculate total price
//         }
//       });
//     }
//   }
...
//                                             _calculateTotalPrice(room
//                                                 .price); // Recalculate total price
//                                           });
//                                         },
//                                       ),
//                                       const SizedBox(height: 12),
...
//                                         ),
//                                         child: Text(
//                                             isSelected ? "Selected" : "Choose", style: TextStyle(color: Colors.white),),
//                                       ),
//                                     ],
//                                   ),
//                                 ),
...
//   List<int> selectedRoomIndices = []; // List untuk menyimpan indeks ruangan yang dipilih
...
//       // Initialize startDates, endDates, and guestCounts lists
//       startDates = List<DateTime?>.filled(_rooms.length, null);
//       endDates = List<DateTime?>.filled(_rooms.length, null);
//       guestCounts = List<int>.filled(_rooms.length, 1); // Default guest count for each room
//     });
//   }
...
//         if (nights > 0) {
//           totalPrice += nights * _rooms[index].price * guestCounts[index];
//         }
//       }
//     }
//     setState(() {});
...
//     final picked = await showDatePicker(
//       context: context,
//       initialDate: endDates[index] ?? startDates[index]!.add(const Duration(days: 1)),
...
//                     mainAxisAlignment: MainAxisAlignment.spaceBetween,
//                     children: [
//                       Text('Total Price: Rp${totalPrice.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold)),
//                       ElevatedButton(
//                         onPressed: () {
//                           // Navigate to booking page
//                         },
...
//                                       // Date Picker for Start Date
//                                       OutlinedButton(
//                                         onPressed: isSelected ? () => _pickStartDate(index) : null,
//                                         child: Text(startDates[index] == null ? 'Pick Start Date' : 'Start: ${startDates[index]!.toLocal().toString().split(' ')[0]}'),
//                                       ),
//                                       const SizedBox(height: 8),
//                                       // Date Picker for End Date
//                                       OutlinedButton(
//                                         onPressed: isSelected ? () => _pickEndDate(index) : null,
//                                         child: Text(endDates[index] == null ? 'Pick End Date' : 'End: ${endDates[index]!.toLocal().toString().split(' ')[0]}'),
//                                       ),
//                                       const SizedBox(height: 8),
...
//                                               setState(() {
//                                                 if (guestCounts[index] > 1) guestCounts[index]--;
//                                                 _calculateTotalPrice(); // Recalculate total price
//                                               });
//                                             } : null,
//                                           ),
...
//                                               controller: TextEditingController(text: guestCounts[index].toString()),
//                                               onChanged: (value) {
//                                                 setState(() {
//                                                   guestCounts[index] = int.tryParse(value) ?? 1; // Default to 1 if parsing fails
//                                                   _calculateTotalPrice(); // Recalculate total price
//                                                 });
//                                               },
//                                             ),
...
//                                       ElevatedButton(
//                                         onPressed: () {
//                                           _toggleRoomSelection(index); // Toggle room selection
//                                         },
//                                         style: ElevatedButton.styleFrom(
//                                           backgroundColor: isSelected ? Colors.blue : Colors.grey[500],
//                                         ),
//                                         child: Text(isSelected ? "Selected" : "Choose", style: const TextStyle(color: Colors.white)),
//                                       ),
//                                     ],
//                                   ),
//                                 ),
